# streamlit_app.py — Backtester with EstVol/ActualVol + Stochastic+Bollinger (auto-invert)
import os, io, time
import numpy as np
import pandas as pd
...
# ----------------------- PAGE CONFIG -----------------------
st.set_page_config(page_title="Srini Backtester (Stoch+BB Auto-Invert)", layout="wide")
...
def bollinger_bands(price: pd.Series, lb: int = 20, mult: float = 2.0):
...
def stochastic_kd(df: pd.DataFrame, k:int=14, d:int=3, smooth:int=3):
    high, low, close = df["High"], df["Low"], df[price_col(df)]
    ll = low.rolling(k).min()
    hh = high.rolling(k).max()
    raw_k = 100 * (close - ll) / (hh - ll + 1e-12)
    k_sm = raw_k.rolling(smooth).mean()
    d_ln = k_sm.rolling(d).mean()
    return k_sm.fillna(0.0), d_ln.fillna(0.0)

def market_regime(price: pd.Series, fast:int=50, slow:int=200) -> pd.Series:
    sma_f = price.rolling(fast).mean()
    sma_s = price.rolling(slow).mean()
    reg = pd.Series(1, index=price.index)   # +1 bull
    reg[sma_f < sma_s] = -1                 # -1 bear
    return reg.fillna(1)
...
def stoch_bb_signals(df: pd.DataFrame,
                     bb_lb:int=20, bb_mult:float=2.0,
                     k:int=14, d:int=3, smooth:int=3,
                     ob:int=80, os:int=20,
                     trend_fast:int=50, trend_slow:int=200,
                     allow_countertrend: bool=True) -> pd.Series:
    """Auto-inverting Stochastic + Bollinger signals using regime filter.
       - Bull regime (+1): prefer long bounces off lower band with %K cross-up < os.
       - Bear regime (-1): prefer short fades off upper band with %K cross-down > ob.
       If allow_countertrend=True, we also allow opposite-side signals but weaker.
    """
    price = df[price_col(df)]
    mid, up, lo = bollinger_bands(price, lb=bb_lb, mult=bb_mult)
    k_line, d_line = stochastic_kd(df, k=k, d=d, smooth=smooth)
    reg = market_regime(price, fast=trend_fast, slow=trend_slow)
    ...
    # Bull regime: long preferred
    bull = reg == 1
    sig[bull & (price <= lo) & cross_up & (k_line < os)] = 1.0
    if allow_countertrend:
        sig[bull & (price >= up) & cross_down & (k_line > ob)] = -1.0

    # Bear regime: short preferred
    bear = reg == -1
    sig[bear & (price >= up) & cross_down & (k_line > ob)] = -1.0
    if allow_countertrend:
        sig[bear & (price <= lo) & cross_up & (k_line < os)] = 1.0
...
def backtest(df: pd.DataFrame, strategy: str, params: dict,
    ...
    elif strategy == "Stoch + Bollinger (Auto-Invert)":
        sig = stoch_bb_signals(
            df,
            bb_lb=params.get("bb_lb",20), bb_mult=params.get("bb_mult",2.0),
            k=params.get("k",14), d=params.get("d",3), smooth=params.get("smooth",3),
            ob=params.get("ob",80), os=params.get("os",20),
            trend_fast=params.get("trend_fast",50), trend_slow=params.get("trend_slow",200),
            allow_countertrend=params.get("allow_countertrend", True)
        )
    else:
    ...
    lev = (vol_target / (est_vol + 1e-12)).clip(upper=5.0)    # leverage cap
    ...
    net_exposure   = pos          # signed leverage
    gross_exposure = pos.abs()    # magnitude
    regime_series  = market_regime(price, fast=params.get("trend_fast",50), slow=params.get("trend_slow",200))

    # P&L by side (use prior position to accrue next bar)
    prev_pos = pos.shift(1).fillna(0.0)
    pnl_long  = pnl[prev_pos > 0].sum()
    pnl_short = pnl[prev_pos < 0].sum()

    # Time in market
    pct_long  = (prev_pos >  0).mean()
    pct_short = (prev_pos <  0).mean()
    pct_flat  = (prev_pos == 0).mean()

    # Sharpe by regime
    sharpe_bull = sharpe(pnl[regime_series ==  1])
    sharpe_bear = sharpe(pnl[regime_series == -1])

    # Avg net exposure by regime
    avg_net_bull = net_exposure[regime_series ==  1].mean()
    avg_net_bear = net_exposure[regime_series == -1].mean()

    # Regime switches
    regime_switches = int((regime_series.diff().fillna(0) != 0).sum())
    ...
    stats = {
        ...
        # Exposure & regime extras
        "%Long": round(float(pct_long), 3),
        "%Short": round(float(pct_short), 3),
        "%Flat": round(float(pct_flat), 3),
        "Sharpe_Bull": round(float(sharpe_bull), 2),
        "Sharpe_Bear": round(float(sharpe_bear), 2),
        "AvgNetExp_Bull": round(float(avg_net_bull), 3) if pd.notna(avg_net_bull) else 0.0,
        "AvgNetExp_Bear": round(float(avg_net_bear), 3) if pd.notna(avg_net_bear) else 0.0,
        "Regime_Switches": regime_switches,
        "PnL_Long": round(float(pnl_long), 4),
        "PnL_Short": round(float(pnl_short), 4),
    }

    debug_df = None; debug_summary = None
    if debug:
        mid, up, lo = bollinger_bands(price, lb=params.get("bb_lb",20), mult=params.get("bb_mult",2.0))
        k_line, d_line = stochastic_kd(df, k=params.get("k",14), d=params.get("d",3), smooth=params.get("smooth",3))
        debug_df = pd.DataFrame({
            ...
            "Stoch_%K": k_line, "Stoch_%D": d_line
...
# ----------------------- UI -----------------------
st.title("📊 Srini Backtester — Stoch+BB (Auto-Invert) + EstVol vs ActualVol")
...
with st.sidebar:
    ...
    strategy = st.selectbox(
        "Strategy",
        ["SMA Crossover", "RSI Mean Reversion", "Composite", "Stoch + Bollinger (Auto-Invert)"],
        index=3
    )
    ...
    if strategy == "SMA Crossover":
        ...
        # Stochastic + Bollinger params
        b1, b2, b3 = st.columns(3)
        bb_lb  = b1.number_input("BB lookback", 5, 100, 20, 1)
        bb_sd  = b2.number_input("BB stdev (×)", 1.0, 3.5, 2.0, 0.1)
        allow_ct = b3.checkbox("Allow counter-trend signals", value=True)

        s1, s2, s3, s4 = st.columns(4)
        k = s1.number_input("%K lookback", 3, 50, 14, 1)
        d = s2.number_input("%D smooth", 1, 20, 3, 1)
        smooth = s3.number_input("%K smoothing", 1, 20, 3, 1)
        ob = s4.number_input("Overbought (≥)", 50, 99, 80, 1)

        s5, s6 = st.columns(2)
        os = s5.number_input("Oversold (≤)", 1, 50, 20, 1)
        trend_fast = s6.number_input("Trend fast SMA", 5, 200, 50, 1)
        trend_slow = st.number_input("Trend slow SMA", 20, 500, 200, 5)

        params = {
            "bb_lb": int(bb_lb), "bb_mult": float(bb_sd),
            "k": int(k), "d": int(d), "smooth": int(smooth),
            "ob": int(ob), "os": int(os),
            "trend_fast": int(trend_fast), "trend_slow": int(trend_slow),
            "allow_countertrend": bool(allow_ct)
        }
    ...
    debug_mode = st.checkbox("Run Debug Mode (log EstVol/ActualVol + Exposure)", value=True)
...
if run_btn:
    ...
    if not data:
        st.error("No data downloaded. Try adding exchange suffixes (.TO/.V for Canada, .NS/.BO for India).")
        st.stop()
    ...
    for t in in_list:
        ...
        if d is None or d.empty:
            hint = "" if "." in t else "Try .TO/.V (Canada) or .NS/.BO (India)."
    ...
    for tab, t in zip(tabs, data.keys()):
        with tab:
            ...
            st.markdown(
                ...
                f"**Exposure (non-flat bars):** {stats['Exposure']:.0%}  |  "
                f"**%Long/Short/Flat:** {stats['%Long']:.0%}/{stats['%Short']:.0%}/{stats['%Flat']:.0%}  |  "
                f"**Sharpe Bull/Bear:** {stats['Sharpe_Bull']:.2f}/{stats['Sharpe_Bear']:.2f}  |  "
                f"**Avg NetExp Bull/Bear:** {stats['AvgNetExp_Bull']:.2f}/{stats['AvgNetExp_Bear']:.2f}  |  "
                f"**Regime Switches:** {stats['Regime_Switches']}  |  "
                f"**PnL L/S:** {stats['PnL_Long']:.3f}/{stats['PnL_Short']:.3f}  |  "
                f"**Entries/Exits/Flips:** {stats['Trades_Entered']}/{stats['Trades_Exited']}/{stats['Flips']}  |  "
                f"**Total Costs:** {stats['TotalCosts(%)']:.3f}%"
            )

            # Optional SMA cross markers for SMA strategy
            if strategy == "SMA Crossover":
    ...
    def _strategy_name_and_params(strategy, params):
        if strategy == "SMA Crossover":
            ...
            return strategy, {
                "BB lookback": params.get("bb_lb"), "BB stdev": params.get("bb_mult"),
                "%K": params.get("k"), "%D": params.get("d"), "%K smooth": params.get("smooth"),
                "OB/OS": f"{params.get('ob')}/{params.get('os')}",
                "Trend SMA fast/slow": f"{params.get('trend_fast')}/{params.get('trend_slow')}",
                "Allow counter-trend": params.get("allow_countertrend")
            }
