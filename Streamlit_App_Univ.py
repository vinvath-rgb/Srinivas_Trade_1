# Streamlit_App_Univ.py
# Filename-agnostic Streamlit app: Universe (ETF/Stocks/Both) + Backtester
# - Send BUY by default; optional WATCH; never AVOID
# - Preview & max-send cap; de-dup
# - No st.switch_page; uses st.session_state to update Backtester
...
def score_universe(data: dict[str, pd.DataFrame],
    ...
    # Momentum blend (use available parts only)
    comp = []
...
with tab_univ:
    ...
    if run_u:
        ...
        # Build list to send (respect action filter + cap)
        ...
        if to_send:
            ...
            preview_placeholder.info("Nothing to send yet — extend date range, lower filters, or enable 'Include WATCH'.")
