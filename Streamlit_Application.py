# streamlit_app.py — Backtester with Est/Actual Vol + Bollinger MR + SMA+BB Filter + Capital logs
import os, io, time
import numpy as np
import pandas as pd
...
st.set_page_config(page_title="Srini Backtester (SMA / RSI / Composite / BB / SMA+BB)", layout="wide")
...
def sma_with_bb_filter(price: pd.Series, fast: int, slow: int, bb_win: int, bb_k: float) -> pd.Series:
    """
    Keep standard SMA crossover signal but require:
    ...
    # Apply filter on the SAME bar (you can shift by 1 if you want fully non-lookahead)
    filt = base.copy()
    long_mask  = base > 0
    short_mask = base < 0
...
def backtest(df: pd.DataFrame, strategy: str, params: dict,
    ...
    # --- Vol targeting -> position sizing (gross leverage) ---
    est_vol = realized_vol(rets, lookback=20)                 # EstVol (past)
    lev = (vol_target / (est_vol + 1e-12)).clip(upper=5.0)    # annualized target
    pos = (sig * lev).fillna(0.0)                             # gross position (can be 0, +lev, -lev)

    # --- Execution with stops/TP produces P&L ---
    ...
    # --- Leverage & invested capital series (logs/metrics) ---
    gross_lev = pos.abs()  # intended gross leverage fraction
    invested_cap = equity.shift(1).fillna(equity.iloc[0]) * gross_lev  # equity-units “capital at work”
    ...
    stats = {
        ...
        "MinCapital": round(min_cap, 4),   # equity units
        "MaxCapital": round(max_cap, 4),   # equity units
    ...
    if debug:
        debug_df = pd.DataFrame({
            ...
            "InvestedCapital": invested_cap,   # equity units
...
# ----------------------- UI -----------------------
st.title("📊 Srini Backtester — EstVol vs ActualVol (SMA / RSI / Composite / Bollinger / SMA+BB Filter)")
...
if run_btn:
    ...
    for tab, t in zip(tabs, data.keys()):
        with tab:
            ...
            # Bollinger chart (for BB MR or SMA+BB)
            if strategy in ("Bollinger Mean Reversion", "SMA + Bollinger Filter"):
                close = df[price_col(df)]
                bbw = params["bb_window"] if "bb_window" in params else 20
                bbk = params["bb_k"] if "bb_k" in params else 2.0
    ...
    # show Min/Max Capital (equity units) as columns
    ...
    for t in res_df.index:
        ...
        # Convert Min/MaxCapital (equity units) to dollars
    ...
    st.download_button("⬇️ Download Backtest Report (Excel, with Vol/Lev/Capital)",
