...
with col1:
    raw_symbol = st.text_input("Symbol", value="SHOP")  # e.g., SHOP (TSX), AAPL (US)
with col2:
...
def fetch_candles(sym: str, res: str, days: int) -> pd.DataFrame | None:
    ...
    params = {"symbol": sym, "resolution": res, "from": _from, "to": now, "token": API_KEY}
...
if st.button("🔌 Test Finnhub Connectivity"):
    ok, msg = ping_finnhub()             # <-- FIX: call the function
